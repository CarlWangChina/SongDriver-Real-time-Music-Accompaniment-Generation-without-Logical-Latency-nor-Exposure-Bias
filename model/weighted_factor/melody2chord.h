...
constexpr bool tone_nh[] = {true, false, true, false, true, true, false, true, false, true, false, true};
constexpr bool tone_1245[] = {true, false, true, false, false, true, false, true, false, false, false, false};
...
#pragma omp parallel for
        for (int i = 0; i < len; ++i) {
            ...
            float compCount = 0;
            auto base = notes.at(i);
            //不能当和弦
            if (!tone_nh[std::get<0>(base) % 12]) {
                continue;
            }
            for (int j = i; j < len; ++j) {
                auto it = notes.at(j);
...
#endif
            //验证是否为调内音
            auto base = std::get<0>(n.at(0));
            if (base != 0) {
                for (auto& delta_n : it.second) {
                    if (!tone_nh[(delta_n + base) % 12]) {
                        goto end;
                    }
                }
            }
            weight = calcEditDist(relative, it.second, act) + baseWeight;
#ifdef MGNR_DEBUG
            printf("=>%f(%f)\n", weight, baseWeight);
#endif
            locker.lock();
...
template <chord_map_c T>
inline std::tuple<int, std::string, std::vector<int>, float> note2Chord(const T& self, const std::vector<std::tuple<int, int>>& seq, float act = 0.5, int times = 1) {
    std::map<int, int> count;
    for (auto it : seq) {
        if (std::get<0>(it) > 0) {
            count[std::get<0>(it)] += std::get<1>(it) * times;
        }
    }
...
template <chord_map_c T>
musicSection_t buildMusicSection(const T& self, const std::vector<std::tuple<int, int>>& melody, float act = 1.0, int times = 1) {
    musicSection_t res;
    res.melody = melody;
